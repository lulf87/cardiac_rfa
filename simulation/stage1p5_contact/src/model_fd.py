...
def interpolate_lesion_depth_mm(omega: np.ndarray, y_m: np.ndarray, threshold: float, centerline: int) -> float:
    line = omega[:, centerline]
    above = np.where(line >= threshold)[0]
    if len(above) == 0:
        return 0.0
    j = int(above.max())
    if j >= len(y_m) - 1:
        return 1e3 * float(y_m[j])
    om0 = float(line[j])
    om1 = float(line[j + 1])
    y0 = float(y_m[j])
    y1 = float(y_m[j + 1])
...
def run_case(cfg: CaseConfig) -> Dict[str, Any]:
    ...
    centerline = cfg.nx // 2
    lesion_depth_mm = interpolate_lesion_depth_mm(omega, y, cfg.lesion_omega_threshold, centerline)
    transmural = lesion_depth_mm >= cfg.wall_thickness_mm - 1e-6
    overheat_area_mm2 = float((peak_T >= 100.0).sum() * dx * dy * 1e6)
...
def plot_case(result: Dict[str, Any], outpath: str | Path) -> None:
    ...
    fig.suptitle(
        ...
        f'insert={cfg.insertion_depth_mm:.2f} mm | depth={depth:.2f} mm',
        fontsize=13,
    )
