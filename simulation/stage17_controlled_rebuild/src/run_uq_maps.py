...
def main() -> None:
    ...
    def summarize_group(g: pd.DataFrame) -> pd.Series:
        n = int(len(g))
        k_trans = int(g['transmural'].sum())
        k_over = int(g['overheat'].sum())
        trans_lo, trans_hi = wilson_interval(k_trans, n)
        over_lo, over_hi = wilson_interval(k_over, n)
        return pd.Series({
            'transmural_probability': float(g['transmural'].mean()),
            'transmural_probability_ci_low': trans_lo,
            'transmural_probability_ci_high': trans_hi,
            'overheat_probability': float(g['overheat'].mean()),
            'overheat_probability_ci_low': over_lo,
            'overheat_probability_ci_high': over_hi,
            'depth_fraction_mean': float(g['depth_fraction'].mean()),
            'depth_fraction_p10': float(np.quantile(g['depth_fraction'], 0.10)),
            'depth_fraction_p50': float(np.quantile(g['depth_fraction'], 0.50)),
            'depth_fraction_p90': float(np.quantile(g['depth_fraction'], 0.90)),
            'lesion_depth_mean_mm': float(g['lesion_depth_mm'].mean()),
            'lesion_width_mean_mm': float(g['lesion_width_mm'].mean()),
            'lesion_area_mean_mm2': float(g['lesion_area_mm2'].mean()),
            'depth_to_width_ratio_mean': float(g['depth_to_width_ratio'].mean()),
            'tmax_mean_C': float(g['peak_temperature_C'].mean()),
            'tmax_p90_C': float(np.quantile(g['peak_temperature_C'], 0.90)),
            'n_samples': n,
        })
    ...
    overview = (
        ...
        .agg(
            ...
            lesion_width_mean_mm=('lesion_width_mean_mm', 'mean'),
            lesion_area_mean_mm2=('lesion_area_mean_mm2', 'mean'),
        )
    )
