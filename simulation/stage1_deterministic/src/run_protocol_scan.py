...
from model_fd import CaseConfig, clone_cfg, run_case, plot_case, save_summary, summarize_result
...
def main():
    ...
    for p in protocols:
        cfg = clone_cfg(base, power_W=float(p['power_W']), duration_s=float(p['duration_s']))
        res = run_case(cfg)
    ...
    ax.set_title('Protocol comparison (deterministic baseline)')
    fig.savefig(outdir / 'protocol_comparison.png', dpi=180)
    plt.close(fig)
    print(df[['name', 'power_W', 'duration_s', 'lesion_depth_mm', 'peak_temperature_C', 'overheat_area_mm2', 'transmural']])
