...
def save(fig, outbase):
    ...
    fig.savefig(outbase + ".tiff", dpi=600, bbox_inches="tight", pil_kwargs={"compression": "tiff_lzw"})
    plt.close(fig)
...
def fig3_deterministic(phase_df: pd.DataFrame, outdir: str):
    ...
    for name in PROTOCOL_ORDER:
        ...
        ax.plot(s["wall_thickness_mm"], s["depth_fraction"], marker=PROTOCOL_MARKER[name], color=colors[name], label=PROTOCOL_LABEL[name])
    ...
    for name in PROTOCOL_ORDER:
        ...
        ax.plot(s["insertion_depth_mm"], s["depth_fraction"], marker=PROTOCOL_MARKER[name], color=colors[name])
    ...
    for name in PROTOCOL_ORDER:
        ...
        ax.plot(s["cooling_h_W_per_m2K"], s["depth_fraction"], marker=PROTOCOL_MARKER[name], color=colors[name])
    ...
    for name in PROTOCOL_ORDER:
        s = sub[sub["name"] == name].sort_values("insertion_depth_mm")
        ax.plot(s["insertion_depth_mm"], s["peak_temperature_C"], marker=PROTOCOL_MARKER[name], color=colors[name])
    ...
    handles = [
        Line2D([0], [0], color=colors[n], marker=PROTOCOL_MARKER[n], lw=2.5, markersize=8, label=PROTOCOL_LABEL[n])
    ...
    for i, protocol in enumerate(rows):
        for j, h in enumerate(cols):
            ...
            if i < len(rows)-1:
                ...
                ax.set_xticklabels([f"{x:.1f}" for x in INSERTION_ORDER])
                ax.set_xlabel("Nominal insertion [mm]")
            if j > 0:
                ax.set_yticklabels([])
            else:
                ax.set_yticklabels([f"{int(y)}" for y in WALL_ORDER])
